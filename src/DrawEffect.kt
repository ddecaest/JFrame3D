...
class RotationEffect(private val rotationMatrixCreationFunction: (Float) -> RotationMatrix) : DrawEffect {
    ...
    companion object {
        ...
        fun rotateX(): RotationEffect {
            return RotationEffect { counter: Float -> RotationMatrixFactory.createXRotationMatrix(counter) }
        }

        fun rotateY(): RotationEffect {
            return RotationEffect { counter: Float -> RotationMatrixFactory.createYRotationMatrix(counter)}
        }

        fun rotateZ(): RotationEffect {
            return RotationEffect { counter: Float -> RotationMatrixFactory.createZRotationMatrix(counter) }
        }
    }
